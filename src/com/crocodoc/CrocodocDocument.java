...
public class CrocodocDocument extends Crocodoc {
	...
	/**
	 * Check the status of a file on Crocodoc by UUID. Takes one UUID string
	 * and returns one status array for that UUID.
	 * 
	 * @param string uuid The uuid of the file to check the status of
	 * 
	 * @return array An array of the uuid, status, and viewable bool, or an
	 *   array of the uuid and an error
	 * @throws CrocodocException
	 */
	public static Map<String, Object> status(String uuid) throws CrocodocException {
		ArrayList<String> uuids = new ArrayList<String>();
		uuids.add(uuid);
		ArrayList<Map<String, Object>> statuses = status(uuids);
		Map<String, Object> status = statuses.get(0);

		if (!status.containsKey("uuid") || status.get("uuid") == null || status.get("uuid") == "") {
	 ...
	 * 
	 * @param array uuids An array of the uuids of the file to check the
	 *   status of
	 * 
	 * @return array An array of the uuid, status, and viewable bool, or an
	 *   array of the uuid and an error
	 * @throws CrocodocException
	 */
	public static ArrayList<Map<String, Object>> status(ArrayList<String> uuids) throws CrocodocException {
		...
		JSONArray response = (JSONArray) _requestJson(path, "status", getParams, null);
	...
	public static String upload(String url) throws CrocodocException {
		...
		JSONObject response = (JSONObject) _requestJson(path, "upload", null, postParams);
	 ...
	 * Upload a file to Crocodoc with a URL.
	 * 
	 * @param object file The file resource to be uploaded
	 * 
	 * @return string The uuid of the newly-uploaded file
	 * @throws CrocodocException
	 */
